    ...
    >
      <template #leftHeaderHandle>
        ...
        >
          ...
          <el-form :model="queryParams" ref="queryFromRef" label-position="top">
            <el-row :gutter="40">
              <el-col :span="24">
      ...
      <template #rightHeaderHandle>
        ...
        >
          ...
          <div class="flex justify-end">
            ...
            <el-button size="small" type="primary" @click="handleRightSearch()">
              查询
            </el-button>
          </div>
        </el-popover>
      ...
      <template #rightFooter>
        <el-button type="primary" @click="onRemoveRight"> 移除考生 </el-button>
      </template>
    </TransferLayout>
...
import BatchQueryOverview from "./BatchQueryOverview.vue";
...
const onDialogSubmit = () => {
  let obj = {
    examineeIds: rightList.value.map((item) => item.id).join(","),
  };
  modifyGroupExamineeApi(obj, props.modifyId)
    .then(() => {
      CmeMessage({
        title: "成功",
        message: "保存成功",
...
const fetchRightList = () => {
  ...
  getGroupExamineeApi({}, props.modifyId)
    .then((res) => {
      rightList.value = res;
