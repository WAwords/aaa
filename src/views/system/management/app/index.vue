<template>
  <div class="router-page">
    ...
    >
      <template #query-form>
        <el-form-item prop="name" label="APP名称" class="prop-name">
          <el-input
        ...
        <el-form-item prop="code" label="APP编码" class="prop-code">
          <el-input
      ...
      <el-table-column label="操作" fixed="right" width="200px" align="center">
        <template #default="{ row }">
          ...
          <el-button
            ...
            v-hasPermi="['system:app-version:list']"
            text
          >
            维护
          </el-button>
    ...
    >
      ...
      >
        <el-row :gutter="40">
          <el-col :span="24">
            <el-form-item label="APP名称" prop="name" class="prop-name">
              <el-input v-model="dialogForm.name" placeholder="请输入APP名称" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="APP编码" prop="code" class="prop-code">
              <el-input
...
import type { AppsListItem, AppsQueryType } from "@/api/system/management/app";
import AppVersion from "./components/AppVersion.vue";

defineOptions({
  name: "SystemManagementApp",
});

const queryParams = ref<AppsQueryType>({
  pageNumber: 1,
  pageSize: 10,
  name: "",
  code: "",
...
const tableData = ref<AppsListItem[]>([]);
const fetch = (i = 0) => {
  loading.value = true;
  if (i !== 0) {
    queryParams.value.pageNumber = i;
  }
  getAppsApi(queryParams.value)
    .then((res) => {
...
const dialogForm = ref<AppsListItem>({
  name: "",
  code: "",
  remark: "",
});
...
const handleEditClick = (row: AppsListItem) => {
  ...
  dialogFromRef.value
    ...
    .then(() => {
      ...
      if (dialogType.value === "create") {
        ...
        putAppsApi(dialogForm.value, editId.value)
          .then(() => {
            CmeMessage({
              title: "成功",
              message: "修改APP成功",
...
const onSelectionChange = (val: AppsListItem[]) => {
...
const handleActionDelete = () => {
  ...
  CmeMessageBox.confirm("确定批量删除APP吗？", "系统提示", {
    distinguishCancelAndClose: true,
    confirmButtonText: "确定",
    cancelButtonText: "取消",
  })
    .then(() => {
      deleteAppsApi(selectionIds.value)
        .then(() => {
          CmeMessage({
            title: "成功",
            message: "删除APP成功",
...
const modifyRow = ref<AppsListItem>();
provide("modifyRow", modifyRow);
/** 维护 */
const handleModifyClick = (row: AppsListItem) => {
  modifyRow.value = row;
  modifyVisible.value = true;
};

/** 操作栏的单个删除 */
const handleDeleteClick = (row: AppsListItem) => {
  CmeMessageBox.confirm(`确定删除APP“${row.name}”吗？`, "系统提示", {
    distinguishCancelAndClose: true,
    confirmButtonText: "确定",
    cancelButtonText: "取消",
  })
    .then(() => {
      deleteAppsApi(row.id + "")
        .then(() => {
          CmeMessage({
            title: "成功",
            message: "删除APP成功",
...
const dialogFormRules = {
  name: [...getFormItemRule("APP名称", 20, -1, true)],
  code: [...getFormItemRule("APP编码", 40, -1, true)],
