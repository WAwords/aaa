<template>
  ...
  <el-scrollbar height="200px">
    <div class="icon-list">
      <div
        class="icon-item"
        :style="{ backgroundColor: activeIcon === i ? '#e5e7eb' : '' }"
        v-for="i in tempIcons"
        @click="handleClick(i)"
      >
        <div class="icon-container">
          <Icon :icon="i" />
        </div>
        <span>{{ i ? i.split(":")[1] : "无图标" }}</span>
      </div>
    </div>
  </el-scrollbar>
</template>
...
props;
const iconName = ref("");
const tempIcons = computed(() => {
  return icons.filter((i) => i.includes(iconName.value));
});
const emit = defineEmits(["update:activeIcon", "selected"]);
const handleClick = (i: string) => {
  emit("update:activeIcon", i);
  emit("selected", i);
  iconName.value = "";
};
